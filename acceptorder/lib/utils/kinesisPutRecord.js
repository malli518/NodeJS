...
const logger = imports.log4js.getLogger('kinesisPutRecord');
logger.setLevel(imports.loggerLevel);

const kinesisPutRecord = function(data, context) {
        ...
        try {
            ...
            if (recordSize <= imports.KINESIS_MAX_PAYLOAD_BYTES) {
                ...
                s3.putObject(params, function(err, data) {
                    if (err) {
                        logger.error(`  ${context.awsRequestId}  kinesisPutRecord err: ${err.stack}`);
                    } else {
            ...
            logger.error(`  ${context.awsRequestId}  kinesisPutRecord exception: ${exception}`);
            reject(exception);
        }
    });
};
module.exports = kinesisPutRecord;